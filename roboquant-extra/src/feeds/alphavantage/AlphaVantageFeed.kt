package org.roboquant.feeds.alphavantage
...
class AlphaVantageFeed(
     ...
     * @param asset
     */
    fun subscribe(asset: Asset, interval: String = "FIVE_MIN") {

        val i = try {
            Interval.valueOf(interval)
        } catch (e: Exception) {
            throw Exception("Supported interval values are ${Interval.values().map { it.toString() }}")
        }
