...
[source, shell]
----
mvn test -Dtest="org.roboquant.PerformanceTest" -DTEST_PERFORMANCE -DPARALLEL=4 -pl roboquant
----
The `-DPARALLEL=4` flag determines how many parallel runs to use in the performance test (the default is 4 is you don't include this flag). The output will look something like this:
