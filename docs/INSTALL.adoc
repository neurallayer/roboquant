...
This will pull the image from DockerHub and run it on your local machine. The image comes with several tutorial-notebooks that demonstrate how to develop and run your own strategies.
...
The build and install ais always tested using JDK 17 runtime, however the generated libraries are targeted against JDK 11 in order to provide greater compatibilty for projects that still use older versions of the JDK.
